...
#         StructField("testTime",StringType()),
#         StructField("totalSteps",StringType()),
...
#TO-DO: create a spark application object and set the spark log level to WARN
...
zSetDecodedEncodedCustomerDF= zSetEntriesEncodedStreamingDF.withColumn("encodedCustomer", unbase64(zSetEntriesEncodedStreamingDF.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
zSetDecodedEncodedCustomerDF\
    .withColumn("customerRecords", from_json("encodedCustomer", customerJSONSchema))\
    .select(col('customerRecords.*'))\
...
# emailAndBirthDayStreamingDF = spark.sql("select reservationId, reservationDate from CustomerRecords where reservationDate is not null")
...
# +--------------------+-----
emailAndBirthYearStreamingDF.writeStream.outputMode("append").option("truncate", False).format("console").start().awaitTermination()
